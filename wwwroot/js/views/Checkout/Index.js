...
function editQtyTemplate(line) {
    ...
    const $numBox = $qtyDiv.dxNumberBox({
        width: String(Math.trunc(line.Quantity)).length * 8 + 108,
...
function determineProduct(line) {
    ...
    $selectBoxes.each(function (_, obj) {
        numberOfAttributes++;
        const selected = $(obj).dxSelectBox().dxSelectBox('instance').option("value");
        if (selected) {
            const productVariation = { Attribute: $(obj).data(DATASET_ATTRIBUTE), Code: selected, Name: selected };
...
function editVariantTemplate(line) {
    ...
    if (line.ChildProductInfo.length) {
        ...
        for (const childProduct of line.ChildProductInfo) {
            for (const productVariation of childProduct.ChildProductAttributes) {
                if ((_a = combinations[productVariation.Attribute]) === null || _a === void 0 ? void 0 : _a.length) {
...
function renderShippingChoice($shipToCustomerDiv) {
    ...
    if (hasFreeShippingPolicy && !isPaymentClicked) {
        ...
        $shipToCustomerDiv.append($('<span class="ml-2">').append($('<span>').text('('), $freeShippingStatus, $('<span>').text(')')));
    }
...
function prepareDeliveryMethodFields(defCallingCode) {
    ...
    if (!availableDeliveryMethods.length) {
        $deliveryMethodFieldset.append(`<div class="font-italic">${vdata.localizer.noDeliveryMethod.replaceAll('{0}', vdata.model.organizationShortName).replaceAll('{1}', vdata.model.organizationEmailAddress)}</div>`);
        return;
    }
    ...
    if (chosenDeliveryMethod === Enums_1.DeliveryOption.NoOption) {
        return;
    }
    $deliveryMethodFieldset.append($('<h5 class="mt-3">').text(vdata.localizer.deliveryMethod), $('<div id="delivery-method">').dxRadioGroup({
        ...
        itemTemplate: function (itemData, _, itemElement) {
            if (itemData === Enums_1.DeliveryOption.SelfPickup) {
                itemElement.append($('<div class="font-weight-bold">').text(translateDeliveryOption(Enums_1.DeliveryOption.SelfPickup)), $('<div class="delivery-method-info my-2">').text(vdata.model.selfPickUpInstruction
        ...
        onValueChanged: function (e) {
            ...
            $('#deliver-to-section').toggle(chosenDeliveryMethod === Enums_1.DeliveryOption.Shipping);
            $('#recipient-section').toggle(chosenDeliveryMethod === Enums_1.DeliveryOption.Shipping);
            adjustDeliveryCharge();
            $('#delivery-charge-amt').text(StaticData.FormatCurrency(actualDeliveryCharge, vdata.model.currencyCode));
            $('#delivery-charge-div').toggle(chosenDeliveryMethod === Enums_1.DeliveryOption.Shipping);
    ...
    function inputValidationForShipping(input) {
        if (chosenDeliveryMethod === Enums_1.DeliveryOption.SelfPickup)
            return true;
        if (input.value == null || input.value == "" || input.value == undefined)
            return false;
        return true;
    ...
    $deliverToDiv.append($('<h5>').addClass('mt-3').text(vdata.localizer.deliverTo), $('<div id="delivery-address-line1" class="mb-2">').dxTextBox({
...
function handleAddProduct() {
    ...
    DevExpress.ui.dialog
        ...
        if (dialogResult) {
            ...
            }).done(() => { location.reload(); }).fail(function (jqXHR, textStatus, errorThrown) {
                if (jqXHR.status != 410) {
                    ...
                    buttons: [{
                            ...
                            onClick: function () {
                                return $.post({
                                    ...
                                    data: { salesOrderID: vdata.model.salesOrderID }
                                }).done(() => { location.reload(); }).fail(corprio.formatError);
                            },
                            type: "danger",
                        }, {
...
function prepareCustomerInfoForm() {
    ...
    $form.on('submit', function (event) {
        ...
        if (chosenDeliveryMethod === Enums_1.DeliveryOption.Shipping) {
...
function translateDeliveryOption(deliveryOption) {
    switch (deliveryOption) {
        ...
        case Enums_1.DeliveryOption.Shipping:
    ...
    if (isPaymentClicked) {
        ...
        switch (vdata.model.chosenDeliveryMethod) {
            ...
            case 'Shipping':
                ...
                break;
            default:
                chosenDeliveryMethod = Enums_1.DeliveryOption.NoOption;
                break;
        }
        ...
        else if (chosenDeliveryMethod === Enums_1.DeliveryOption.Shipping) {
...
function editProduct() {
    ...
    if (!(relevantOrderLine === null || relevantOrderLine === void 0 ? void 0 : relevantOrderLine.length)) {
        return;
    } // note: empty array is truthy, even though ([] == false) is evaluated as true
    $("#edit-popup").dxPopup({
...
function prepareCartTable() {
    ...
    for (const line of orderLines) {
        ...
        $('<td class="text-left d-flex">').appendTo($row).append($('<div class="flex-even mr-2">').html(line.ProductName), $('<div class="flex-even mr-2">').append((!isPaymentClicked && ((_a = line.ChildProductInfo) === null || _a === void 0 ? void 0 : _a.length) > 1)
