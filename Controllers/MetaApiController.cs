...
namespace Corprio.SocialWorker.Controllers
{
    ...
    {                
        ...
        public MetaApiController(IConfiguration configuration, IHttpClientFactory httpClientFactory) : base()
        {            
        ...
        protected async Task<bool> TurnOnNLP(string accessToken, MetaLanguageModel language)
        {
        ...
        protected string GenerateImageUrlForDev()
        {
            ...
            {
                "https://app.gobuy.click/images/Accessories.png",
                "https://app.gobuy.click/images/Shoes.png",                
            };
            var rand = new Random();
        ...
        protected async Task<string> MakeIgItemContainer(string igUserId, string accessToken, IgMediaType mediaType,
        ...
        protected async Task<string> PublishIgMedia(string igUserId, string accessToken, string mediaContainerId)
        {
            dynamic queryParams = new ExpandoObject();
            queryParams.access_token = accessToken;
        ...
        protected async Task<string> MakeIgCarouselPost(string accessToken, string igUserId, List<string> mediaUrls, string message)
        {
            if (mediaUrls == null)
            {
            ...
            foreach (string mediaUrl in mediaUrls)
            {
                ...
                string mediaContainerId = await MakeIgItemContainer(igUserId: igUserId, accessToken: accessToken,
            ...
            if (cache.Count == 1)
            {
                return await MakeIgNonCarouselPost(igUserId: igUserId, accessToken: accessToken,
                    mediaUrl: cache.Select(x => x.Value).ToList().FirstOrDefault(), message: message, mediaType: IgMediaType.Image);
            }

            string carouselId = await MakeIgItemContainer(igUserId: igUserId, accessToken: accessToken,
            ...
            return await PublishIgMedia(igUserId: igUserId, accessToken: accessToken, mediaContainerId: carouselId);
        }
        ...
        /// <returns>ID of the media item posted</returns>
        protected async Task<string> MakeIgNonCarouselPost(string accessToken, string igUserId, string mediaUrl, string message, IgMediaType mediaType)
        {
            string mediaContainerId = await MakeIgItemContainer(igUserId: igUserId, accessToken: accessToken,
                mediaUrl: mediaUrl, caption: message, mediaType: mediaType);
            if (string.IsNullOrWhiteSpace(mediaContainerId)) return string.Empty;

            string mediaId = await PublishIgMedia(igUserId: igUserId, accessToken: accessToken,
        ...
        protected async Task<string> MakeFbMultiPhotoPost(string accessToken, string pageId, List<string> imageUrls, string message)
        {
            if (imageUrls == null)
            {
